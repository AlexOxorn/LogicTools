...
type inference =
  ...
  | AndElimLeft
  | PairElimLeft
  | AndLeft1 of string * string
  | AndElimRight
  | PairElimRight
  | AndLeft2 of string * string
  | AndElimAlt of string * string
  | LinAndProdElim of string * string
  | LetElim of string
  | AndIntro
  | PairConstructor
  | AndRight
  ...
  | VariantIntroRight
  | OrRight2
  | OrElim of string * string
  | OrLeft of string * string * string
  | VariantElim
  (* Implication *)
  | ImplIntro of string
  | LambdaIntro of string
  | ImplRight of string
  | LinImplIntro of string
  | ImplElim
  | ApplicationElimination
  | ImplLeft of string * string
  | LinImplElim
  (* Negations *)
  ...
  (* For All / Exists *)
  | ForAllIntro of string
  | ForAllRight of string
  | ForAllElim
  | ForAllLeft of string * string * string
  | ExistsIntro
  | ExistsRight
  | ExistsElim of string * string
  | ExistsLeft of string * string * string
  (* Box *)
  | BoxIntro
  | BoxElim of string
  | BangIntro
...
type step =
  | Inference of inference * proof list
  | Deduction of deductionName
  | NoInference

and proof = { con : context; term : stmt; inf : step ref }
