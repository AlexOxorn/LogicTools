...
let prove_with_assumption name p =
  p.inf := with_assumption name;;
...
module BaseSolver(Ctx: ContextFunctions) = 
  struct
    (* GENERATING NAMES *)
    let get_used_names (c: context) =
    let rec recursive (c: context) = match c with
      | ConApp (sub, StmtAssumption(a, _))
      | ConApp (sub, VariableAssumption(a, _)) -> StringSet.add a (recursive sub)
    ...
    | [] -> let used = get_used_names c in
            let newname = List.find (fun element -> not (StringSet.mem element used)) variable_names
            in (newname, names) ;;
    ...
    let make_partial_e c e j = {con=c; term=make_term e j; inf=ref NoInference};;
    let make_empty = make_partial_t Ctx.empty
    
    let wrap_context (name: string option) (c: context) = match name with
    | None -> c
    | Some x -> Ctx.name x c
  ;;
...
module NatDedSolver(Ctx: ContextFunctions) = 
  struct
    include BaseSolver(Ctx) 

    let split ?(version=1) ?ctxName ?(vars=[]) (p: proof) = match p with
    ...
    | {con=c; term={exp=Impl(l, r); judge=j}; _} ->
      ...
      let new_ctx = wrap_context ctxName (Ctx.add new_var (make_term l j) c) in
      let to_return = [make_partial_e new_ctx r j] in
      p.inf := Inference (ImplIntro new_var, to_return);
      to_return
    | {con=c; term={exp=Not(e); judge=j}; _} ->
        let pName, pExp, vars = (match version, vars with
           | 1, targetname::rest1 -> (targetname, Name targetname, rest1)
           | _ -> ("\\bot", Bottom, vars)
          ) in
        let (new_var, _) = (find_valid_name vars c) in
        let new_ctx = wrap_context ctxName (Ctx.add new_var (make_term e j) c) in
    ...
    | {con=c; term={exp=NAnd(l, r); judge=j}; _} ->
        (match vars with
          | targetname::rest1 -> (
            ...
            let new_ctx1 = wrap_context None (Ctx.add new_var1 (make_term l j) c) in
            let new_ctx2 = wrap_context ctxName (Ctx.add new_var2 (make_term r j) new_ctx1) in
            let to_return = [make_partial_e new_ctx2 (Name targetname) j] in
            p.inf := Inference (NAndIntro (targetname, new_var1, new_var2), to_return);
            to_return)
    ...
    | {con=c; term={exp=Nor(l, r); judge=j}; _} ->
        (match vars with
          | target1::target2::rest1 -> (
            ...
            let (new_var2, _) = (find_valid_name rest2 c) in
            let new_ctx1 = wrap_context None (Ctx.add new_var1 (make_term l j) c) in
            let new_ctx2 = wrap_context None (Ctx.add new_var2 (make_term r j) c) in
            let to_return = [
              make_partial_e new_ctx1 (Name target1) j;
              make_partial_e new_ctx2 (Name target2) j
            ] in
            p.inf := Inference (NorIntro (target1, new_var1, target2, new_var2), to_return);
            to_return)
          | _ -> failwith "Too Few Names"
        )
    ...
    let proof_by_conta ?ctxName ?(vars=[]) (p: proof) = match p with
    | {con=c; term={exp=e; judge=j}; _} -> (match vars with
      | targetname::rest1 -> (
        let (new_var, _) = (find_valid_name rest1 c) in
        let new_ctx = wrap_context ctxName (Ctx.add new_var (make_term (Not(e)) j) c) in
...
module ModalSolver(Ctx: ContextFunctions) = struct
  ...
  let _split_ctx ctx = match ctx with
      | ConCat(l, r) -> (l, r)
      | _ -> failwith "Failed to Split Context"

  let split ?ctxName ?(vars=[]) (p: proof) = match p with
    ...
    | {con=c; term={exp=CtxExp(sub);_};_ } ->
      ...
      in
      let to_return = (Ctx.flatten sub)
       |> List.map _extract_statment
       |> List.map (make_partial_t c)
      in
        p.inf := Inference (CtxProof, to_return);
      to_return
    | _ -> Super.split ?ctxName:ctxName ~vars:vars p;;
  
  let unbox ctx name new_name ?(newJudge=Valid) ?ctxName p =
      let (_, r) = _split_ctx ctx in
      let stmt = match Ctx.lookupStmt name r with
      | Some {exp=Box(NoContext, e, _); _} -> {exp=e; judge=newJudge}
      | Some {exp=Box(c, e, _); _} -> {exp=Box(c, e, Valid); judge=newJudge}
      | _ -> failwith "Variable is not boxed"
      in
      let box_proof = use_assumption ctx name in
      let ctx2 = match ctxName with
      | Some cName -> ModalContext.(add ~alt:ValidCTX new_name stmt ctx |> nameLeft cName)
      | None ->  ModalContext.add ~alt:ValidCTX new_name stmt ctx in
  ...
  let assumption_elimination var_name ({con=c;_} as p) =
      ...
      let (cubC, eTerm) = match Ctx.lookupStmt var_name l with
      | Some {exp=Box(c2, e, Valid); _} -> c2, e
      | _ -> failwith "Variable is Found in Valid Context" in
      if not (Equality.expr_eq_b eTerm p.term.exp) then failwith "Expression Doesn't Match"
      else
...
module SeqSolver(Ctx: ContextFunctions) = 
  struct
    include BaseSolver(Ctx) 
    let split_right ?(v=1) ?ctxName ?(vars=[]) (p: proof) = match p with
      | {con=c; term={exp=Impl(l, r); judge=j}; _} ->
        let (new_var, _) = (find_valid_name vars c) in
        let new_ctx = wrap_context ctxName (Ctx.add new_var (make_term l j) c) in
        let to_return = [make_partial_e new_ctx r j] in
      ...
      | {con=c; term={exp=And(l, r); judge=j}; _} ->
        let to_return = [
          make_partial_e c l j;
          make_partial_e c r j;
        ] in
    ...
    let split_left ?(v=1) (names: (string option * string list) list) (variable) (p: proof) = 
      let stmt = Ctx.lookupStmt variable p.con in
        match names, p, stmt with
        | (ctxName, vars)::_, {con=c; term={exp=e; judge=j}; _}, Some({exp=And(l, r); _}) ->
          let (new_var, _) = (find_valid_name vars c) in
          let new_ctx = wrap_context
                        ctxName
                        (Ctx.add new_var 
                                    (make_term (if v = 1 then l else r) j) 
                                    c
                        ) in
          let to_return = [make_partial_e new_ctx e j] in
          p.inf := Inference ((if v = 1 then AndLeft1 (variable, new_var) else AndLeft2 (variable, new_var)), to_return);
          to_return
        | (ctxName1, vars1)::
          (ctxName2, vars2)::_,
          ...
          let new_ctx1 = wrap_context ctxName1 (Ctx.add new_var1 (make_term l j) c) in
          let new_ctx2 = wrap_context ctxName2 (Ctx.add new_var2 (make_term r j) c) in
          let to_return = [make_partial_e new_ctx1 e j;
                          make_partial_e new_ctx2 e j] in
          p.inf := Inference ((OrLeft (variable, new_var1, new_var2)), to_return);
          to_return
        | (ctxName, vars)::_, {con=c; term={exp=e; judge=j}; _}, Some({exp=Impl(l, r); _}) ->
          let (new_var1, _) = (find_valid_name vars c) in
          let new_ctx1 = wrap_context ctxName (Ctx.add new_var1 (make_term r j) c) in
          let to_return = [make_partial_e new_ctx1 e j;
                          make_partial_e c l j] in
          p.inf := Inference ((ImplLeft (variable, new_var1)), to_return);
          to_return
        | (None, [a])::(ctxName, vars)::_, {con=c; term={exp=e; judge=j}; _}, Some({exp=ForAll(x, t0, pred); _}) ->
          let sub_expr = (
            match Ctx.lookupStmt a c with 
            Some ({judge=TypeOf t1; exp=s}) -> if t1 == t0 then s else failwith (Format.asprintf "variable %s is not the correct type" a)
...
module LinearSolver(Ctx: ContextFunctions) = struct
  ...
  let _split_ctx ctx = match ctx with
      ...
      let split ?(ctxName=[]) ?(vars=[]) ?(extract_names=[]) (p: proof) = match p with
    | {con=c; term={exp=LinImpl(l, r); judge=j}; _} ->
      let (new_var, _) = (find_valid_name vars c) in
      let new_ctx = wrap_context (List.nth_opt ctxName 0) (Ctx.add new_var (make_term l j) c) in
      let to_return = [make_partial_e new_ctx r j] in
  ...
  let unbang ctx name new_name ?(newJudge=Valid) ?(ctxName=[]) p =
      ...
      let ctx1 = match List.nth_opt ctxName 2 with
      | Some cName -> ModalContext.(ll |> nameRight cName)
      | None -> ll in
      let ctx2 = match List.nth_opt ctxName 0 with
      | Some cName -> ModalContext.(rr |> add ~alt:ValidCTX new_name stmt |> nameLeft cName)
      | None ->  ModalContext.(rr |> add ~alt:ValidCTX new_name stmt) in
      let ctx3 = match List.nth_opt ctxName 1 with
