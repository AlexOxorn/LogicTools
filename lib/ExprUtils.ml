...
let expr_bin_judge_combine (ee: expr -> expr -> expr) (jj: judgement -> judgement -> judgement) =
  let combined { exp=le; judge=lj } { exp=re; judge=rj } = { exp=(ee le re) ; judge=(jj lj rj)}
in combined;;

let expr_uni_judge_combine (ee: expr -> expr) (jj: judgement -> judgement) =
  let combined { exp=le; judge=lj } = { exp=(ee le) ; judge=(jj lj)}
in combined;;

let forward (l: judgement) (r: judgement) = if l == r then l else (failwith "Incompaitble judgements");;


let wrap_type_combination2 (tf: ty -> ty -> ty) =
  let res (j1: judgement) (j2: judgement) = match (j1, j2) with
  | TypeOf a, TypeOf b -> TypeOf(tf a b)
  | _ -> (failwith "Incompaitble judgements")
in res;;
let wrap_type_combination1 (tf: ty -> ty) =
  let res (j1: judgement) = match j1 with
  | TypeOf a -> TypeOf(tf a)
  | _ -> (failwith "Incompaitble judgements")
in res;;

let extractLeft (t:ty) = match t with
| Sum(a, _) | Prod(a, _) | Func(a, _) -> a
| _ -> failwith "Incomaptible Type for ExtractLeft";;
let extractRight (t:ty) = match t with
...
module CurryExpr = struct
  ...
  let (?<) = expr_uni_judge_combine ( ?<- ) (wrap_type_combination1 extractLeft);;
  let (?>) = expr_uni_judge_combine ( ?>- ) (wrap_type_combination1 extractRight);;
  let top = Utils.is_type top_ UnitType

  let lam (var: string) (t: ty) = expr_uni_judge_combine (fun a -> Lambda(var, a)) (wrap_type_combination1 ((=>) t))
  let (/=>) a b = lam (fst a) (snd a) b
  
  let ( @ ) l r = match (l, r) with 
  | {exp=l; judge=TypeOf(Func(_, b1))}, {exp=r; judge=TypeOf(_)} -> {exp=l @- r; judge=TypeOf(b1)}
  | _ -> failwith "Left Side is not Function Type"
  
  let injL t = expr_uni_judge_combine (fun a -> InjectLeft(t, a)) (wrap_type_combination1 (fun l -> l + t))
  let injR t = expr_uni_judge_combine (fun a -> InjectRight(t, a)) (wrap_type_combination1 ((+) t))
  let caseStmt m a na b nb = 
    let tp = (match na, nb with 
      | {exp=_; judge=TypeOf(c)}, {exp=_; judge=TypeOf(c2)} -> if Equality.type_eq_b c c2 then c else failwith "Incompatible Types"
      | _ -> failwith "Incompatible Types") in
    match m, na, nb with
    | {exp=mE; judge=_}, {exp=naE; judge=_}, {exp=nbE; judge=_} ->
...
include NatDedExp
  ...
  let (|=>) (c: context) (s: stmt) = {exp=Box(c, s.exp, s.judge); judge=s.judge};;
end
