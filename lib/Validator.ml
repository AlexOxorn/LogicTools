...
let judge_eq =
  wrap_eq_res (fun x y -> JudgementNotEqual (x, y)) Equality.judge_eq_b

let expr_eq = wrap_eq_res (fun x y -> ExprNotEqual (x, y)) Equality.expr_eq_b

let judgement_eq =
  wrap_eq_res (fun x y -> JudgementNotEqual (x, y)) Equality.judgement_eq_b

let stmt_eq = wrap_eq_res (fun x y -> StmtNotEqual (x, y)) Equality.stmt_eq_b

let assumption_eq =
  wrap_eq_res (fun x y -> AssumptionNotEqual (x, y)) Equality.assumption_eq_b

let context_strict_equal =
  wrap_eq_res
    (fun x y -> ContextNotEqual (x, y))
    Equality.context_strict_equal_b

let context_equivalent =
  wrap_eq_res (fun x y -> ContextNotEqual (x, y)) Context.equivalent

let assumption_in_context =
  wrap_eq_res (fun x y -> NotInContext (y, x)) Equality.assumption_in_context_b

let extractDataFromInference ({ con = c; term = s; inf = i; _ } : proof) =
  match (s, !i) with
  | { exp = e; judge = j }, Inference (r, p) -> (c, e, r, p, j)
  | { exp = _; judge = Truth }, NoInference
  | { exp = _; judge = Truth }, Deduction _ ->
      (c, Top, Axiom, [], Truth)
...
let check_flat_correctness (p : proof) =
  ...
  (* TOP LEVEL *)
  | _, Top, Axiom, [], _ -> Ok ()
  | c, e, Assumption a, [], NoJudge ->
      assumption_in_context (StmtAssumption (a, no_judge e)) c
  | c, e, Assumption a, [], Truth ->
      assumption_in_context (StmtAssumption (a, is_true e)) c
  | c, e, ValidAssumption a, [], Truth ->
      assumption_in_context (StmtAssumption (a, is_valid e)) c
  | c, _, Assumption a, [], TypeOf t ->
      assumption_in_context (VariableAssumption (a, TypeOf t)) c
  (* AND INTRO *)
  | c, And (l, r), AndIntro, [ l_proof; r_proof ], Truth ->
      expr_eq l_proof.term.exp l
  ...
  | ( c,
      e,
      AndElimLeft,
      [ { con = c1; term = { exp = And (sube, _); _ }; _ } ],
      Truth )
  | ( c,
      e,
      AndElimRight,
      [ { con = c1; term = { exp = And (_, sube); _ }; _ } ],
      Truth )
  | ( c,
      Or (e, _),
      OrIntroLeft,
      [ { con = c1; term = { exp = sube; _ }; _ } ],
      Truth )
  | ( c,
      Or (_, e),
      OrIntroRight,
      [ { con = c1; term = { exp = sube; _ }; _ } ],
      Truth ) ->
      context_strict_equal c c1 &&@ lazy (expr_eq sube e)
  (* OR ELINIMATION *)
  | ( c,
      e,
      OrElim (x, y),
      ...
      &&@ lazy
            (context_strict_equal
               (ConApp (c, StmtAssumption (x, is_true l)))
               cl)
      &&@ lazy
            (context_strict_equal
               (ConApp (c, StmtAssumption (y, is_true r)))
               cr)
  (* IMPLICATION *)
  | ( c,
      Impl (pre, post),
      ImplIntro name,
      [ { con = c1; term = { exp = post1; _ }; _ } ],
      Truth ) -> (
  ...
  | ( c,
      Not e,
      NotIntro (targetname, varname),
      [ { con = c1; term = { exp = Name tmpname; _ }; _ } ],
      Truth ) ->
      targetname =@ tmpname
      &&@ lazy
            (context_strict_equal
               (ConApp (c, StmtAssumption (varname, is_true e)))
               c1)
  | ( c,
      _,
      NotElim,
  ...
  | ( c,
      e,
      ByContra (targetname, varname),
      [ { con = c1; term = { exp = Name tmpname; _ }; _ } ],
      Truth ) ->
      targetname =@ tmpname
      &&@ lazy
            (context_strict_equal
               (ConApp (c, StmtAssumption (varname, is_true (Not e))))
               c1)
  (* Custom Nand *)
  | ( c,
      NAnd (l, r),
      NAndIntro (targetname, lname, rname),
      [ { con = c1; term = { exp = Name tmpname; _ }; _ } ],
      Truth ) ->
      targetname =@ tmpname
      &&@ lazy
            (context_strict_equal
               (ConApp
                  ( ConApp (c, StmtAssumption (lname, is_true l)),
                    StmtAssumption (rname, is_true r) ))
               c1)
  | ( c,
      Name _,
      NAndElim,
...
let rec check_above_proofs (p : proof) =
  match !(p.inf) with
  | Inference (_, subproofs) ->
      List.to_seq subproofs
      |> Seq.map (fun a -> lazy (check_truth_correctness a))
      |> Seq.fold_left ( &&@ ) (Ok ())
  | _ -> Ok ()
