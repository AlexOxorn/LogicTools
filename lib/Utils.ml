...
let _use_assumption assumption_type func con ?st name =
  match st with
  | Some t -> { con; term = func t; inf = ref (assumption_type name) }
  | None -> (
      match Context.lookupStmt name con with
      | Some t -> { con; term = func t; inf = ref (assumption_type name) }
      | None -> failwith "Can't Find Assumption in Context")

let use_assumption = _use_assumption with_assumption Fun.id

let use_valid_ass ?(map_judge = to_true) =
  _use_assumption with_valid_assumption map_judge

let make_proof con target rule proofs =
  { con; term = target; inf = ref (Inference (rule, proofs)) }

let make_condition con target = { con; term = target; inf = ref NoInference }

let make_deduction con target deduction =
  { con; term = target; inf = ref (Deduction deduction) }
