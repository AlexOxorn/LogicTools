...
let precedence = function
  ...
  | LinZero ->
      0
  | Pair _ -> 1
  | Not _ | Predicate _ | Box _ | Application _ | First _ | Second _
  | NextTime _ | Bang _ | InjectLeft _ | InjectRight _ | EvaluationContext _ ->
      2
...
let type_is_atomic (e : ty) =
  match e with NamedType _ | UnitType | BottomType -> true | _ -> false
...
let rec expr_latex (e : expr) =
  let rec inner wrap (e : expr) =
    let base =
      ...
      | Bottom -> "\\bot"
      | And (l, r) ->
          Format.asprintf "%s \\land %s"
            (inner (le_prec e l) l)
            (inner (le_prec e r) r)
      | Or (l, r) ->
          Format.asprintf "%s \\lor %s"
            (inner (le_prec e l) l)
            (inner (le_prec e r) r)
      | Impl (l, r) ->
          Format.asprintf "%s \\supset %s"
            (inner (le_prec e l) l)
            (inner (lt_prec e r) r)
      | Not n -> Format.asprintf "\\lnot %s" (inner (lt_prec e n) n)
      | NAnd (l, r) ->
          Format.asprintf "%s \\mathbin{\\overline{\\land}} %s"
            (inner (le_prec e l) l)
            (inner (le_prec e r) r)
      | Nor (l, r) ->
          Format.asprintf "%s \\mathbin{\\overline{\\lor}} %s"
            (inner (le_prec e l) l)
            (inner (le_prec e r) r)
      (* Curry Howard *)
      | TypeUnit -> "()"
      | Pair (l, r) ->
          Format.asprintf "\\langle %s, %s \\rangle" (inner false l)
            (inner false r)
      ...
      | Application (l, r) ->
          Format.asprintf "%s\\ %s"
            ...
            (inner (le_prec e r) r)
      | Lambda (arg, body) ->
          Format.asprintf "\\lambda %s.%s" arg (inner (lt_prec e body) body)
      | Case (pred, (x1, b1), (x2, b2)) ->
          Format.asprintf
            "\\text{Case } %s \\text{ of inj}_l\\ %s \\Rightarrow %s \\ \
             |\\text{ inj}_r\\ %s \\Rightarrow %s"
            (inner (le_prec e pred) pred)
            x1
            (inner (le_prec e b1) b1)
            x2
            (inner (le_prec e b2) b2)
      | InjectLeft (t, n) ->
          Format.asprintf "\\text{inj}_l^{%s} %s"
            (type_latex ~strict:false t)
            (inner (le_prec e n) n)
      | InjectRight (t, n) ->
          Format.asprintf "\\text{inj}_r^{%s} %s"
            (type_latex ~strict:false t)
            (inner (le_prec e n) n)
      | LetPair (l, r, p, b) ->
          Format.asprintf "\\text{let } %s = %s \\text{ in } %s"
            (inner false (Pair (Name l, Name r)))
            (inner (lt_prec e p) p)
            (inner (le_prec e b) b)
      (* First Order *)
      | Predicate (n, e) ->
          Format.asprintf "%s(%s)" n
            (e |> List.map (inner false) |> String.concat ", ")
      | ForAll (n, NoType, e) ->
          Format.asprintf "\\forall %s.%s" n (inner false e)
      | Exists (n, NoType, e) ->
          Format.asprintf "\\exists %s.%s" n (inner false e)
      | ForAll (n, t, e) ->
          Format.asprintf "\\forall %s{:}%s.%s" n (type_latex t) (inner false e)
      | Exists (n, t, e) ->
          Format.asprintf "\\exists %s{:}%s.%s" n (type_latex t) (inner false e)
      (* Modal *)
      | Let (u, n, m) ->
          Format.asprintf "\\text{let } %s = %s \\text{ in } %s" u
            (inner false n) (inner false m)
      | Box (NoContext, n, _) ->
          Format.asprintf "\\square %s" (inner (lt_prec e n) n)
      | Box (c, e, Valid) ->
          Format.asprintf "[%s]"
            (context_stmt_latex_base " \\Vdash " c (Utils.no_judge e))
      | Box (c, e, _) ->
          Format.asprintf "[%s]"
            (context_stmt_latex_base " \\vdash " c (Utils.no_judge e))
      ...
      | LinZero -> "\\mathbb{0}"
      | LinAndProd (l, r) ->
          Format.asprintf "%s \\otimes %s"
            (inner (le_prec e l) l)
            (inner (le_prec e r) r)
      | LinAndSum (l, r) ->
          Format.asprintf "%s \\mathbin{\\&} %s"
            (inner (le_prec e l) l)
            (inner (le_prec e r) r)
      | LinOrProd (l, r) ->
          Format.asprintf "%s \\oplus %s"
            (inner (le_prec e l) l)
            (inner (le_prec e r) r)
      | LinOrSum (l, r) ->
          Format.asprintf "%s \\iamp %s"
            (inner (le_prec e l) l)
            (inner (le_prec e r) r)
      | LinImpl (l, r) ->
          Format.asprintf "%s \\multimap %s"
            (inner (le_prec e l) l)
            (inner (lt_prec e r) r)
    ...
    let base =
      ...
      | Func (l, r) ->
          Format.asprintf "%s \\rightarrow %s"
            (inner (ty_le_prec t l) l)
            (inner (ty_lt_prec t r) r)
      | NoType -> if strict then failwith "Try to print NoType Latex" else ""
    in
    wrap_paran (wrap && not (type_is_atomic t)) base
  in
  inner false t

and judgement_latex (j : judgement) =
  match j with
  | NoJudge -> ""
  | Truth -> " \\text{ true}"
  | Up -> "^{\\uparrow}"
...
and assumption_latex (ass : assumption) =
  match ass with
...
let inference_latex (i : inference) =
  match i with
  (* Axioms *)
  | Axiom -> ""
  | Law x -> "\\text{" ^ x ^ "}"
  ...
  | NAndIntro (p, l, r) ->
      Format.asprintf "\\overline{\\land} I^{%s, %s, %s}" p l r
  | NorIntro (p, l, q, r) ->
      Format.asprintf "\\overline{\\lor} I^{%s, %s, %s, %s}" p l q r
...
let rec print_latex_deduction_name (d : deductionName) =
  ...
  | Substitution (x, y) ->
      let substitutions =
        y
        |> List.map (fun (x, y) ->
            Format.asprintf "%s/%s" (print_latex_deduction_name x) y)
        |> String.concat ","
      in
      Format.asprintf "[%s]%s" substitutions (print_latex_deduction_name x)

let rec proof_list_latex_base connection (prfs : proof list) =
  prfs |> List.map (proof_latex_base connection) |> String.concat " & "
...
and proof_latex_base connection (p : proof) =
  ...
  match !(p.inf) with
  | Inference (i, prfs) ->
      Format.asprintf "\\infer [%s] {%s} {%s}" (valid inference_latex i)
        (context_stmt_latex_base connection p.con p.term)
        (proof_list_latex_base connection prfs)
  | Deduction s ->
      Format.asprintf "\\deduce [] {%s} {%s}"
        (context_stmt_latex p.con p.term)
        (print_latex_deduction_name s)
  | NoInference -> context_stmt_latex p.con p.term

and wrap_valid v s x =
  if v then s x else Format.asprintf "{\\color{red} %s}" (s x)
...
let rec find_opt k map =
  match map with
  | (target, dest) :: tl -> if target = k then Some dest else find_opt k tl
  | [] -> None

let notation named_ctx =
  match named_ctx with
  | ConNameWithDef (target, ctx) ->
      Format.asprintf "  %s &= %s " target (context_latex ctx)
...
let rec addCtxToMap (map : ctxMap) named_ctx =
  ...
  | ConNameWithDef (target, sub) -> (
      ...
      | Some _ -> map
      | None ->
          let new_map = (target, notation named_ctx) :: map in
          addCtxToMap new_map sub)
  | ConApp (c, _) -> addCtxToMap map c
  | ConCat (l, r) -> addCtxToMap (addCtxToMap map l) r
  | _ -> map

let generateCtxMap (p : proof) =
  let rec searchExp (e : expr) (map : ctxMap) =
    match e with
    | And (l, r) | NAnd (l, r) | Or (l, r) | Impl (l, r) ->
