...
let rec trans (e : expr) : expr =
  ...
  | Case (e, (s1, e1), (s2, e2)) ->
      Case (trans e, (s1, trans e1), (s2, trans e2))
  | Let (u, n, m) -> Application (Lambda (u, trans m), trans n)
  | _ -> failwith "out of scope"

let proof_con1 e r ps c =
  { con = c; term = { judge = Truth; exp = e }; inf = ref (Inference (r, ps)) }
...
module TranslationTools = struct
  ...
  let rec find_in_map (var : string) l : (context -> proof) option =
    match l with
  ...
  let replace_judge (p : judgement -> bool) nJ ({ exp = e; judge = j } as s) =
    match p j with true -> { exp = e; judge = nJ } | false -> s
  ...
  let cleanse_context =
    cleanse_context_maker ~assPred:(fun _ -> false) ~assMap:Fun.id

  let assumption_or_proof ?(cleanser = cleanse_context) ?(stmtMap = Fun.id) map
      c var =
    ...
    if Option.is_some prf_opt then Option.get prf_opt
    else
      match con_opt with
      | Some (StmtAssumption (_, s)) -> use_assumption con var ~st:(stmtMap s)
      | Some (VariableAssumption (n, j)) ->
          make_proof con { exp = Name n; judge = j } (Assumption n) []
      | _ ->
          failwith
            ("Can't find " ^ var ^ " in " ^ print_context con ^ " or "
...
module SeqTranslatorTools = struct
  ...
  let replace_true_ass = function
    | StmtAssumption (x, { exp = e; _ }) ->
        StmtAssumption (x, { exp = e; judge = Truth })
    | VariableAssumption (x, _) -> VariableAssumption (x, Truth)

  let cleanse_context =
    cleanse_context_maker ~assPred:is_stmt_ass ~assMap:replace_true_ass

  let assumption_or_proof =
    TranslationTools.assumption_or_proof ~cleanser:cleanse_context
...
module CurryTranslatorTools = struct
  ...
  let termNames =
    List.init 13 (fun i -> Char.escaped (Char.chr (int_of_char 'M' + i)))

  let typeNames =
    List.init 12 (fun i -> Char.escaped (Char.chr (int_of_char 'A' + i)))

  let get_new_term_name (used : StringSet.t) =
    let newName =
      List.find (fun element -> not (StringSet.mem element used)) termNames
    in
    (newName, StringSet.add newName used)

  let get_new_type_name (used : StringSet.t) =
    let newName =
      List.find (fun element -> not (StringSet.mem element used)) typeNames
    in
    (newName, StringSet.add newName used)

  let rec expr_to_type ?default e =
    match e with
    | Top -> UnitType
    | And (l, r) -> Prod (expr_to_type ?default l, expr_to_type ?default r)
    | Or (l, r) -> Sum (expr_to_type ?default l, expr_to_type ?default r)
    | Impl (l, r) -> Func (expr_to_type ?default l, expr_to_type ?default r)
  ...
  let replace_ass ?default = function
    | StmtAssumption (x, { exp = e; _ }) ->
        VariableAssumption (x, TypeOf (expr_to_type ?default e))
    | VariableAssumption _ as v -> v

  let cleanse_context ?default =
    cleanse_context_maker ~assPred:is_stmt_ass ~assMap:(replace_ass ?default)
...
let rec seq_to_nat (p : proof) : proof =
  ...
  let rec seq_to_nat_with_map map
      ({ con = c0; term = { judge = _; exp = e }; inf = i } : proof) : proof =
    let c = cleanse_context map c0 in
    ...
    | Inference (OrRight1, pfs) ->
        proof_con1 e OrIntroLeft (map_sub_proofs pfs) c
    | Inference (OrRight2, pfs) ->
        proof_con1 e OrIntroRight (map_sub_proofs pfs) c
    | Inference (ForAllRight x, pfs) ->
        proof_con1 e (ForAllIntro x) (map_sub_proofs pfs) c
    | Inference (ImplRight x, pfs) ->
        proof_con1 e (ImplIntro x) (map_sub_proofs pfs) c
    | Inference (ImplLeft (u, v), [ pl; pr ]) ->
        ...
        let b_proof =
          proof_con1 b_stmt.exp ImplElim [ impl_proof; a_proof ] c
        in

        let c_proof = seq_to_nat pl in
        let b_to_c_proof =
          proof_con1 (Impl (b_stmt.exp, e)) (ImplIntro v) [ c_proof ] c
        in
    ...
    | Inference (AndLeft1 (x, y), [ pf ]) ->
        ...
        in
        seq_to_nat_with_map
          (add_to_map y construct_proof map)
          (replace_context c pf)
    ...
    | Inference (ForAllLeft (src, rep, dest), [ pup ]) ->
        ...
        in
        seq_to_nat_with_map
          (add_to_map dest construct_proof map)
          (replace_context c pup)
    | NoInference | Deduction _ ->
        { con = c; term = { judge = Truth; exp = e }; inf = i }
    | _ -> failwith "Invalid Seq Inference"
  in
...
let nat_to_curry ?(strict = true) (p : proof) : proof =
  ...
  let rec nat_to_curry_with_used (used : StringSet.t)
      ({ con = c0; term = { judge = _; exp = e }; inf = i } : proof) :
      StringSet.t * proof =
    ...
    | Inference (AndIntro, [ pfL; pfR ]) ->
        ...
        ( used3,
          make_proof trPR.con
            CurryExpr.(trPL.term & trPR.term)
            PairConstructor [ trPL; trPR ] )
    ...
    | Inference (AndElimRight, [ pf ]) ->
        let used2, trP = nat_to_curry_with_used used pf in
        ( used2,
          make_proof trP.con CurryExpr.(?>(trP.term)) PairElimRight [ trP ] )
    | Inference (ImplIntro x, [ pf ]) ->
        let used2, trP = nat_to_curry_with_used used pf in
        let var_ass =
          match Context.lookupAss x trP.con with
          | Some (VariableAssumption (n, TypeOf t)) -> (n, t)
          | _ -> failwith "Var not in context"
        in
        ( used2,
          make_proof c CurryExpr.(var_ass /=> trP.term) (LambdaIntro x) [ trP ]
        )
    | Inference (ImplElim, [ pfl; pfr ]) ->
        ...
        ( used3,
          make_proof c
            CurryExpr.(trPL.term @ trPR.term)
            ApplicationElimination [ trPL; trPR ] )
    | Inference (OrIntroLeft, [ pf ]) ->
        let sumTy = expr_to_type e in
        let used2, trP = nat_to_curry_with_used used pf in
        ( used2,
          make_proof trP.con
            CurryExpr.(injL (extractRight sumTy) trP.term)
            VariantIntroLeft [ trP ] )
    | Inference (OrIntroRight, [ pf ]) ->
        let sumTy = expr_to_type e in
        let used2, trP = nat_to_curry_with_used used pf in
        ( used2,
          make_proof trP.con
            CurryExpr.(injR (extractLeft sumTy) trP.term)
            VariantIntroRight [ trP ] )
    | Inference (OrElim (a, b), [ pf1; pf2; pf3 ]) ->
        ...
        ( used4,
          make_proof c
            CurryExpr.(caseStmt trP1.term a trP2.term b trP3.term)
            VariantElim [ trP1; trP2; trP3 ] )
        ...
        ( used2,
          {
            ...
            term =
              {
                judge = TypeOf (expr_to_type ?default:default_type e);
                exp = Name term;
              };
            inf = i;
          } )
          ...
          ( used1,
            {
              ...
              term =
                {
                  judge = TypeOf (expr_to_type e ~default:(NamedType t));
                  exp = Name x;
                };
              inf = ref (Inference (Axiom, []));
            } )
  in
