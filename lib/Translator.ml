...
let rec substitue oldname new_expr expr =
  let rr = substitue oldname new_expr in
  match expr with
  ...
  | Lambda (arg, body) -> Lambda (arg, (rr body))
...
let rec substitue_proofs oldname new_expr {con=c; term={judge=j;exp=e}; inf=i} = match !i with
  | Inference (i, pfs) ->
    {con=c;
    term={judge=j;exp=(substitue oldname new_expr e)};
    inf=(ref (Inference(i, (List.map (substitue_proofs oldname new_expr) pfs))))}
  | _ ->
    {con=c;
    term={judge=j;exp=(substitue oldname new_expr e)};
    inf=i}
