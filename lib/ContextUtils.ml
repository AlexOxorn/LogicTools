...
module type ContextFunctions = sig
  ...
  val extract_names: ?leftName:string -> ?rightName:string -> string list -> t -> t * t
...
module Context : ContextFunctions = struct
  ...
  let remove_by pred ?ctxMap con = 
    let rec inner pred ctxMap con = match con with
    | ConName _
    | Empty
    | NoContext-> con, false
    | ConApp (c, a) ->
  ...
  let rec transform_by pred func con = match con with
    | ConName _
    | Empty
    | NoContext-> con
    | ConApp (c, a) ->
    ...
    | ConCat (c1, c2) ->
      ConCat((transform_by pred func c1),
            (transform_by pred func c2))
    | ConNameWithDef (x, c) -> ConNameWithDef(x, transform_by pred func c);;
  
  let transform_all = transform_by (fun _ -> true);;

  let rec _find pred post con = match con with
    | ConName _
    | Empty
    | NoContext-> None
    | ConApp (c, a) ->
      if pred a then Some (post a) else _find pred post c      
    | ConCat (c1, c2) ->
  ...
  let collapse con = 
    let rec collapse_step context = match context with
    ...
    | ConApp (c, s) ->
      ConApp(collapse_step c, s)
    | ConCat (Empty, Empty) -> Empty
    | ConCat (Empty, c)
    | ConCat (c, Empty) -> c
    | ConCat (c1, c2) ->
      ConCat(collapse_step c1,
            collapse_step c2)
    | _ -> context in
  ...
  let toMap c = 
    let rec inner m c = match c with
      | NoContext
      | Empty
      | ConName _ -> m
      | ConApp (c, a) -> inner (StmtMap.add (_getname a) (_assToStmt a) m) c
      | ConCat (l, r) -> StmtMap.union (fun _ a _ -> Some a) (inner m l) (inner m r)
      | ConNameWithDef (_, c) -> inner m c
    in inner StmtMap.empty c

  let rec equal (l: context) (r: context) = match l, r with
  | (ConName a), (ConName b) -> a = b
  | (ConApp (c1, a1)), (ConApp (c2, a2)) -> (Equality.assumption_eq_b a1 a2) && (equal c1 c2)
  | (ConCat (c1, a1)), (ConCat (c2, a2)) -> (equal a1 a2) && (equal c1 c2)
  | (ConNameWithDef (n1, c1)), (ConNameWithDef (n2, c2)) -> n1 == n2 && (equal c1 c2)
  | Empty, Empty
  | NoContext, NoContext -> true
  | (ConNameWithDef (_, c1)), x
  | x, (ConNameWithDef (_, c1)) -> (equal c1 x)
  | _ -> false;;
  
  let equivalent (l: context) (r: context) = StmtMap.equal (=) (toMap l) (toMap r)

  let rec latex assPrinter ctx = match ctx with
  | Empty -> "\\,\\cdot\\,"
  | ConName a -> a
  | ConApp (Empty, r) -> Format.asprintf "%s" (assPrinter r)
  | ConApp (l, r) -> Format.asprintf "%s,\\ %s" (latex assPrinter l) (assPrinter r)
  | ConCat (l, r) -> Format.asprintf "%s,\\ %s" (latex assPrinter l) (latex assPrinter r)
    ...
    | ConNameWithDef (x, c) -> 
      if pred = x then 
        ...
        match ctxMap with
          ...
          ConNameWithDef(x, sub), p
    in fst(inner pred ctxMap con);;

  let rec extract ?ctxMap (ref: t) (self: t) = match ref with
    | NoContext
    | Empty -> self
    | ConName n -> remove_ctx_name ?ctxMap:ctxMap n self
    | ConApp(c, a) -> 
      let b = extract ?ctxMap:ctxMap c self in
      remove_by (Equality.assumption_eq_b a) ?ctxMap:ctxMap b
    | ConCat(l, r) ->
      let b = extract ?ctxMap:ctxMap l self in
      extract ?ctxMap:ctxMap r b
    | ConNameWithDef(x, d) ->
      let p = remove_ctx_name ?ctxMap:ctxMap x self in
      extract ?ctxMap:ctxMap d p
  
  let extract_names ?leftName ?rightName (names: string list) (self: t) =
    let rec inner (names: string list) (self: t) = match self with
    | NoContext
    | ConName _
    | Empty     -> Empty, self
    ...
    | ConApp(c, (StmtAssumption (s, _) as a)) -> 
      ...
      if (List.mem s names) then ConApp(l, a), r
      else l, ConApp(r, a)
    | ConCat(l, r) ->
    ...
    | ConNameWithDef(_, d) ->
      inner names d
    in
    let c1, c2 = inner names self
    in match leftName, rightName with
...
module ModalContext = struct
  ...
  let add ?(alt=Auto) n s c = 
    match alt, c, s with
  | Auto, (ConCat(l, r)), {judge=Valid; _}
  | ValidCTX, ConCat(l, r), _ ->
      ConCat((Context.add n s l), r)
  | Auto, (ConCat(l, r)), _ 
  | HypCTX, ConCat(l, r), _ -> ConCat(l, (Context.add n s r))
  | _ -> failwith "Modal Context Must Have Top-level Context Concatenation"
  
  let add_var ?alt:_ n s c = match c, s with
  ...
  let nameLeft  n c = match c with
  | ConCat(l, r) -> ConCat(Context.name n l, r)
  | _ -> failwith "Modal Context Must Have Top-level Context Concatenate"
  let nameRight n c = match c with
  ...
  let latex assPrinter ctx = match ctx with
  | ConCat (l, r) -> Format.asprintf "%s;\\ %s" (Context.latex assPrinter l) (Context.latex assPrinter r)
  | _ -> failwith "Modal Context Must Have Top-level Context Concatenate";;

  let extract_names ?leftName ?rightName names c = match c with
  | ConCat(l, r) ->
    let c1, c2 = Context.extract_names ?leftName:leftName ?rightName:rightName names r in
...
module TemporalContext = struct
  ...
  let next x = 
    let (l, _) = _split x in ConCat(Empty, l)
