...
let rec literal_expr ?(i=1) (e: expr) = match e with
...
| Impl (l, r) -> Format.asprintf "IMPL(\n%s%s,\n%s%s)" (indent i) (literal_expr ~i:(i+1) l) (indent i) (literal_expr ~i:(i+1) r)
| Not e -> Format.asprintf "NOT(%s)" (literal_expr ~i:(i+1) e)
| TypeUnit -> "()"
| Pair (l, r) -> Format.asprintf "PAIR(\n%s%s,\n%s%s)" (indent i) (literal_expr ~i:(i+1) l) (indent i) (literal_expr ~i:(i+1) r)
| First e -> Format.asprintf "FST(%s)" (literal_expr ~i:(i+1) e)
| Second e -> Format.asprintf "SND(%s)" (literal_expr ~i:(i+1) e)
| Application (l, r) -> Format.asprintf "APP(\n%s%s,\n%s%s)" (indent i) (literal_expr ~i:(i+1) l) (indent i) (literal_expr ~i:(i+1) r)
| Lambda (x, r) -> Format.asprintf "LAM(\n%s%s, %s)" (indent i) (x) (literal_expr ~i:(i+1) r)
| InjectLeft (_, r) -> Format.asprintf "INJL(%s)" (literal_expr ~i:(i+1) r)
| InjectRight (_, r) -> Format.asprintf "INJR(%s)" (literal_expr ~i:(i+1) r)
| LetPair (x, y, p, m) -> Format.asprintf "LET_PAIR(\n%s%s,%s,\n%s%s,\n%s%s)" 
    (indent i) x y
    (indent i) (literal_expr ~i:(i+1) p)
    (indent i) (literal_expr ~i:(i+1) m)
| Case (x, (ln, lb), (rn, rb)) -> Format.asprintf "CASE(\n%s%s,\n%s%s, %s,\n%s%s, %s)" 
    (indent i) (literal_expr ~i:(i+1) x)
    (indent i) ln (literal_expr ~i:(i+1) lb)
    (indent i) rn (literal_expr ~i:(i+1) rb)
...
| LetPair (x, y, p, m) -> Format.asprintf "let (%s, %s) = (%s) in (%s)" x y (pp_expr p) (pp_expr m)
| Case (x, (ln, lb), (rn, rb)) -> Format.asprintf "case (%s) { L(%s) -> %s | R(%s) -> %s}" (pp_expr x) ln (pp_expr lb) rn (pp_expr rb)
